...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <header class="w3-container" style="padding-top:22px">
    <h5><b><i class="fa fa-dashboard"></i>Data Guru Piket</b></h5>
  </header>
    <div class="w3-container">
    <?php
  ...
  <!-- copy -->
       <form class="navbar-form navbar-right" role="search" method="POST" action="">
          <input type="text" class="form-control" placeholder="Cari Berdasarkan ID Guru Piket" style="display: block; border: none; border-bottom: 1px solid #808080; float: right; outline: none; height: 30px" name="cari">
      </form>
<!-- end -->
<div class="w3-container">
<a href="input_gurupiket.php"><button class="btn btn-success fa fa-pencil"> Input Guru Piket</button></a> <a href="aksi/cetak_piket.php"><button class="btn btn-success fa fa-lock"> Cetak Password</button></a>  <a href="aksi/cetak_jadwalpiket.php"><button class="btn btn-success fa fa-lock"> Jadwal Guru Piket</button></a>
 </div>
...
<tbody>
  ...
  if(!empty($_POST['cari'])){
$cari=$_POST['cari'];
$query=mysql_query("SELECT * FROM `gurpiket` WHERE id_gurpik LIKE '%$cari%'");//mencari data sesuai id yang dicari
$cek=mysql_num_rows($query);//cek yang dicari ada atau tidak
if($cek==0){
      echo "<p class='alert alert-info'>Data Tidak Ditemukan</p>
<a class='btn btn-default' href=piket.php>Kembali</a>      ";//jika data tidak ditemukan
      }else{
        $q1=mysql_query("SELECT * FROM `gurpiket` WHERE id_gurpik LIKE '%$cari%' LIMIT $st, $per_page");
        $q2=mysql_query("SELECT * FROM `gurpiket` WHERE id_gurpik LIKE '%$cari%'");}
      }
else{
  $q1=mysql_query("SELECT * FROM `gurpiket` LIMIT $st, $per_page");
 $q2=mysql_query("SELECT * FROM `gurpiket`"); }
  
while($data=mysql_fetch_array($q1)){ ?>
    <tr>
    <td><?php echo $data['id_gurpik'];?></td>
    <td><?php echo $data['nama_gurpik'];?></td>
    <td><?php echo $data['status'];?></td>
    <td><?php echo $data['pikethari'];?></td>
    <td><?php echo $data['username'];?></td>
    <td><a href="edit_gurupiket.php?id=<?php echo $data['id_gurpik'];?>" class="fa fa-edit btn btn-success"></a></td>
    <td><a href="aksi/delete_gurupiket.php?id=<?php echo $data['id_gurpik'];?>" class="fa fa-trash btn btn-warning" name="delete" onclick="javascript: return confirm('Anda yakin hapus Data ini ?')"></a></td>
</tr>
